...
log = logging.getLogger('overlayfs')


class OverlayfsHiddenFolder:
    def __init__(self, upper_dir, dry_run):
    ...
    def clean_remote(self, name, remote):
        ...
        try:
            ...
            log.exception("Exception cleaning hidden(s) from %r: ", self.upper_dir)
    ...
    def __files(self):
        ...
        try:
            ...
            log.info("Found %d hidden files in %r", len(hidden_files), self.upper_dir)
        except Exception:
            log.exception("Exception finding hidden files for %r: ", self.upper_dir)
            hidden_files = None
        return hidden_files
    ...
    def __folders(self):
        ...
        try:
            ...
            log.exception("Exception finding hidden folders for %r: ", self.upper_dir)
            hidden_folders = None
        return hidden_folders

    def __hidden2remote(self, remote, hidden_path):
        try:
            remote_path = hidden_path.replace(self.upper_dir, remote['hidden_remote'])
